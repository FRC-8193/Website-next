...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "flex flex-col gap-6 rounded-lg border border-zinc-200 bg-white p-6 text-black shadow-sm transition-all duration-300 hover:shadow-md",
        className,
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("leading-tight font-bold text-black", className)}
      {...props}
    />
  );
}
